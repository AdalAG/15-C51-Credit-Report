...
\newcommand{\coverpage}{%
  ...
  {\fontfamily{phv}\fontsize{10}{12}\selectfont
   \color{NavyReport}
   Adalberto Acuna Girault $\cdot$ Shuchen He $\cdot$ Jingxing Zou}
  ...
  \begin{minipage}[t]{0.55\linewidth}
    ...
    {\fontfamily{phv}\fontsize{9}{12}\selectfont\color{NavyReport}
     ...
     15.C51 $\cdot$ Modeling with ML: Financial Technology\\[1pt]
...
\subsection{Model Selection, Training, and Performance}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{figures/fig1_beeswarm.png}
...
Figure~\ref{fig:pdpice} reveals that A11 and A8 both exert positive but distinctly nonlinear effects on predicted approval probability. A11 exhibits a pronounced threshold pattern: small increases at low values produce a sharp rise in predicted probability, after which the effect saturates quickly. This suggests that A11 primarily acts as a gating variable. Once a minimal level is reached, additional increases provide little incremental information to the model. In contrast, A8 shows a smoother, monotonic increase without a clear saturation point, indicating a more continuously informative role across its range.


The ICE curves for both features display substantial dispersion, highlighting significant heterogeneity in their effects across applicants. This divergence implies that the marginal impact of A11 and A8 is context-dependent, likely driven by interactions with other features. In particular, the same feature value can correspond to materially different prediction changes depending on the broader feature profile, which would be obscured by the population-average PDP alone.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig4_pdp_ice.png}
...
Analysis notebook and related data processing was performed with the help of Claude Code. The AI was used for data cleaning, creating a data pipeline, debugging. 

This written report was developed with the assistance of Perplexity AI (powered by Claude Sonnet 4.6). The AI was used for report drafting, structural planning, internal consistency review, and interpretability framing. 
All analytical conclusions are grounded in the notebook outputs produced by the authors. All figures are produced directly by the analysis pipeline.

The full prompt histories, conversation logs, and pipeline used 
are available in the repository:
\begin{itemize}
  \item \href{https://github.com/AdalAG/15-C51-Credit-Report/blob/main/ai\_logs/claude\_conversation.rtf}{Claude -- Conversation Log}
  \item \href{https://github.com/AdalAG/15-C51-Credit-Report/blob/main/ai\_logs/Perplexity\_conversation.md}{Perplexity -- Conversation Log}
  \item \href{https://github.com/AdalAG/15-C51-Credit-Report/blob/main/Claude\_Pipeline.md}{Claude generated pipeline -- Conversation Log}
  \item \href{https://github.com/AdalAG/15-C51-Credit-Report/blob/main/ai\_logs/other_prompts.txt}{ChatGPT -- Prompts}
